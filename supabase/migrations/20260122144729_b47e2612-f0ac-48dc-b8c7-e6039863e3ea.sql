...
-- Drop both function signatures to eliminate ambiguity
...
-- Create single function with all parameters having defaults
...
BEGIN
  ...
  -- Allow either Supabase auth user or local staff user
  ...
  v_is_admin := COALESCE(
    ...
    OR public.has_role(v_user_id, 'manager'::public.app_role)
    OR public.has_role(v_user_id, 'store_admin'::public.app_role),
    false
  );

  -- Check admin status for local staff users
  v_is_staff_admin := COALESCE(
    public.staff_has_role(p_staff_user_id, 'super_admin'::public.app_role)
    OR public.staff_has_role(p_staff_user_id, 'manager'::public.app_role)
    OR public.staff_has_role(p_staff_user_id, 'store_admin'::public.app_role),
    false
  );

  -- For local staff (cashiers/waitstaff), verify bar assignment
  IF p_staff_user_id IS NOT NULL AND NOT v_is_staff_admin THEN
    IF public.staff_has_role(p_staff_user_id, 'cashier'::public.app_role) 
       OR public.staff_has_role(p_staff_user_id, 'waitstaff'::public.app_role) THEN
      IF NOT EXISTS (
        SELECT 1 FROM public.cashier_bar_assignments cba
  ...
  IF v_user_id IS NOT NULL AND NOT v_is_admin THEN
    IF public.has_role(v_user_id, 'cashier'::public.app_role) 
       OR public.has_role(v_user_id, 'waitstaff'::public.app_role) THEN
      IF NOT EXISTS (
        SELECT 1 FROM public.cashier_bar_assignments cba
  ...
  SELECT bi.current_stock INTO v_current_stock
  FROM public.bar_inventory bi
  WHERE bi.bar_id = p_source_bar_id AND bi.inventory_item_id = p_inventory_item_id
  FOR UPDATE;
  ...
  IF p_admin_complete OR v_is_admin OR v_is_staff_admin THEN
    ...
    DO UPDATE SET current_stock = bar_inventory.current_stock + EXCLUDED.current_stock, updated_at = now();

    v_status := 'completed';

    INSERT INTO public.bar_to_bar_transfers (
      source_bar_id, destination_bar_id, inventory_item_id, quantity, notes,
      status, requested_by, approved_by, completed_at, updated_at
    ) VALUES (
      p_source_bar_id, p_destination_bar_id, p_inventory_item_id, p_quantity, p_notes,
      v_status, v_requester_id, v_requester_id, now(), now()
...
BEGIN
  ...
  IF v_transfer.status <> 'pending' THEN
    RETURN jsonb_build_object('success', false, 'message', 'Transfer already processed', 'status', v_transfer.status);
  END IF;
  ...
  -- Verify destination bar assignment for non-admins
  IF p_staff_user_id IS NOT NULL AND NOT v_is_staff_admin THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.cashier_bar_assignments cba
      WHERE cba.staff_user_id = p_staff_user_id
  ...
  IF p_response = 'accepted' THEN
    ...
    SET status = 'completed', approved_by = v_responder_id, completed_at = now(), updated_at = now()
    WHERE id = p_transfer_id;

    RETURN jsonb_build_object('success', true, 'transfer_id', v_transfer.id, 'status', 'completed');
  ELSE
    ...
    UPDATE public.bar_to_bar_transfers
    SET status = 'rejected', approved_by = v_responder_id, completed_at = now(), updated_at = now()
    WHERE id = p_transfer_id;

    RETURN jsonb_build_object('success', true, 'transfer_id', v_transfer.id, 'status', 'rejected');
  END IF;
END;
$$;
